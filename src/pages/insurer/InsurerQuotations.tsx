...
export default function InsurerQuotations() {
  ...
  // Filter out undefined values to avoid sending empty query params
  ...
  const {
    ...
    error,
    refetch,
  } = useGetQuotationRequestsQuery(filteredQueryParams);
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      const errorMessage =
        ...
        (err as CustomError)?.data?.message ||
        (err as CustomError)?.data?.error ||
  ...
  if (isError) {
    const errorMessage =
      (error as CustomError)?.error?.data ||
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          ...
          {quotationsData?.meta?.total_count || quotationsData?.data?.length
            ? ` (${
